...
class PATH:
    ...
    template_path = os.path.join(SetTing().ServerDir, 'templates')
    config_tmp_path = os.path.join(template_path, 'beaut_template.html')
...
class DIYReportTestResult(ReportTestResult):
    ...
    def get_testcase_property(test) -> tuple:
        ...
        return class_name, method_name, method_doc, start_time, html_path
...
class DIYReport(DIYReportTestResult, PATH):
    img_path = 'img/' if platform.system() != 'Windows' else 'img\\'

    def __init__(self, suites):
        super(DIYReport, self).__init__(suites)
        self.suites = suites
    ...
    def report(self, description, filename: str = None, report_dir='.', log_path=None, theme='theme_default'):
        ...
        :param theme: 报告主题名 theme_default
        :return:
        """
        if log_path:
            import warnings
    ...
    def output_report(self, theme):
        ...
        template_path = self.config_tmp_path  # base目录下的templates
